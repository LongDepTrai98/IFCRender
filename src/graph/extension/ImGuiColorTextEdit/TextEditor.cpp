...
static bool TokenizeJSONString(const char* in_begin, const char* in_end, const char*& out_begin, const char*& out_end)
{
	if (*in_begin != '"')
		return false;
	...
	while (p < in_end)
	{
		if (*p == '\\' && (p + 1 < in_end)) // Ký tự escape (ví dụ: \", \\, \n, \t, \u1234)
		{
			p += 2; // Bỏ qua cả ký tự escape `\` và ký tự tiếp theo
		}
		else if (*p == '"') // Kết thúc chuỗi JSON
		{
			out_begin = in_begin;
			out_end = p + 1; // Bao gồm cả dấu nháy kép đóng "
			return true;
		}
		else
		{
	...
	return false; // Nếu không tìm thấy dấu " đóng, chuỗi JSON không hợp lệ
}

static bool TokenizeJSONNumber(const char* in_begin, const char* in_end, const char*& out_begin, const char*& out_end)
{
	const char* p = in_begin;
	...
	// Ký hiệu khoa học (e hoặc E)
	if (p < in_end && (*p == 'e' || *p == 'E'))
	{
		p++;
		if (p < in_end && (*p == '+' || *p == '-')) // Có thể có dấu + hoặc -
			p++;

		while (p < in_end && isdigit(*p)) // Số mũ
		{
			p++;
			has_digits = true;
		}
...
static bool TokenizeJSONPunctuation(const char* in_begin, const char* in_end, const char*& out_begin, const char*& out_end)
{
	static const std::unordered_set<char> punctuation = { '{', '}', '[', ']', ',', ':' };

	if (in_begin < in_end && punctuation.find(*in_begin) != punctuation.end())
	{
		out_begin = in_begin;
		out_end = in_begin + 1;
		return true;
...
static bool TokenizeJSONIdentifier(const char* in_begin, const char* in_end, const char*& out_begin, const char*& out_end)
{
	static const std::unordered_set<std::string> keywords = { "true", "false", "null" };

	const char* p = in_begin;
	while (p < in_end && isalpha(*p)) // Chỉ chứa chữ cái
		p++;
...
static bool TokenizeJSNumber(const char* in_begin, const char* in_end, const char*& out_begin, const char*& out_end)
{
	const char* p = in_begin;

	if (*p == '-' || *p == '+') // Dấu âm hoặc dương
		p++;

	bool has_digits = false;

	// Hệ nhị phân, bát phân, thập lục phân
	if (p < in_end - 1 && *p == '0' && (*(p + 1) == 'x' || *(p + 1) == 'X' || *(p + 1) == 'b' || *(p + 1) == 'B'))
	{
		p += 2;
		while (p < in_end && isxdigit(*p))
		{
			p++;
			has_digits = true;
		}
...
static bool TokenizeJSPunctuation(const char* in_begin, const char* in_end, const char*& out_begin, const char*& out_end)
{
	static const std::unordered_set<char> punctuation = { '{', '}', '[', ']', '(', ')', ',', ';', '.', ':', '?' };
...
static bool TokenizeJSIdentifier(const char* in_begin, const char* in_end, const char*& out_begin, const char*& out_end)
{
	const char* p = in_begin;
	if (!isalpha(*p) && *p != '_')
		return false;

	while (p < in_end && (isalnum(*p) || *p == '_'))
		p++;
...
static bool TokenizeJSString(const char* in_begin, const char* in_end, const char*& out_begin, const char*& out_end)
{
	if (*in_begin != '"' && *in_begin != '\'' && *in_begin != '`')
		return false;
	...
	while (p < in_end)
	{
		if (*p == '\\' && (p + 1 < in_end)) // Ký tự escape (\n, \t, \", \')
		{
			p += 2;
		}
		else if (*p == quote) // Kết thúc chuỗi
		{
			out_begin = in_begin;
			out_end = p + 1;
			return true;
...
const TextEditor::LanguageDefinition& TextEditor::LanguageDefinition::JSON()
{
	...
	if (!inited)
	{
		// JSON chỉ có 3 từ khóa: true, false, null
		static const char* const jsonKeywords[] = { "true", "false", "null" };
		for (auto& k : jsonKeywords)
			langDef.mKeywords.insert(k);

		// Danh sách các kiểu dữ liệu JSON
		static const char* const identifiers[] = { "object", "array", "string", "number", "boolean", "null" };
		for (auto& k : identifiers)
		{
			Identifier id;
			id.mDeclaration = "JSON Data Type";
			langDef.mIdentifiers.insert(std::make_pair(std::string(k), id));
		}

		// Quy tắc Tokenization để phân biệt Key và Value
		langDef.mTokenize = [](const char* in_begin, const char* in_end, const char*& out_begin, const char*& out_end, PaletteIndex& paletteIndex) -> bool
			{
				paletteIndex = PaletteIndex::Max;
				...
				else if (TokenizeJSONString(in_begin, in_end, out_begin, out_end))
				{
					// Kiểm tra xem chuỗi có phải là "Key" hay không (Key luôn nằm trước dấu `:`)
					const char* next = out_end;
					while (next < in_end && isblank(*next)) next++; // Bỏ qua khoảng trắng
					if (next < in_end && *next == ':')
						paletteIndex = PaletteIndex::Keyword;  // Màu của Key JSON
					else
						paletteIndex = PaletteIndex::String;   // Màu của Value JSON (nếu là chuỗi)
				}
				else if (TokenizeJSONNumber(in_begin, in_end, out_begin, out_end))
					paletteIndex = PaletteIndex::Number; // Số JSON
				else if (TokenizeJSONPunctuation(in_begin, in_end, out_begin, out_end))
					paletteIndex = PaletteIndex::Punctuation; // Dấu câu `{ } [ ] , :`
				else if (TokenizeJSONIdentifier(in_begin, in_end, out_begin, out_end))
					paletteIndex = PaletteIndex::Identifier; // Từ khóa `true`, `false`, `null`

				return paletteIndex != PaletteIndex::Max;
			};

		// Cấu hình comment (JSON không hỗ trợ comment, nhưng có thể thêm để dễ debug)
		langDef.mCommentStart = "/*";
		langDef.mCommentEnd = "*/";
		langDef.mSingleLineComment = "//";
...
const TextEditor::LanguageDefinition& TextEditor::LanguageDefinition::JS()
{
	...
	if (!inited)
	{
		...
		// Hàm chuẩn trong JavaScript
		static const char* const identifiers[] = {
			"alert", "console", "document", "window", "parseInt", "parseFloat", "setTimeout", "setInterval",
			"clearTimeout", "clearInterval", "JSON", "Math", "Date", "RegExp", "Promise", "fetch"
		};
		for (auto& k : identifiers)
		{
			Identifier id;
			id.mDeclaration = "JavaScript Built-in Function";
			langDef.mIdentifiers.insert(std::make_pair(std::string(k), id));
		}

		// Quy tắc Tokenization
		langDef.mTokenize = [](const char* in_begin, const char* in_end, const char*& out_begin, const char*& out_end, PaletteIndex& paletteIndex) -> bool
			{
				paletteIndex = PaletteIndex::Max;
				...
				else if (TokenizeJSString(in_begin, in_end, out_begin, out_end))
					paletteIndex = PaletteIndex::String;
				else if (TokenizeJSNumber(in_begin, in_end, out_begin, out_end))
					paletteIndex = PaletteIndex::Number;
				else if (TokenizeJSPunctuation(in_begin, in_end, out_begin, out_end))
					paletteIndex = PaletteIndex::Punctuation;
				else if (TokenizeJSIdentifier(in_begin, in_end, out_begin, out_end))
					paletteIndex = PaletteIndex::Identifier;
