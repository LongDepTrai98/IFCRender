...
namespace dragon
{
	...
	public:
		WebIFCConverter();
		//const std::map<int, std::vector<IGeometryCache::offset>>& getGeometryOffsetCache();
		~WebIFCConverter();
	public:
		...
		std::unordered_map<int, std::vector<IFCModelCache::offset>>& getGeometryOffset();
	private:
		/*PORT FROM IFC API*/
	...
	private:
		...
		std::unordered_map<int, std::vector<IFCModelCache::offset>> geometry_offset_with_expressID{};
		int vertex_offset{ -1 };
		int index_offset{ -1 };
	};
}
